...
`closed`/`resolved` as tags duplicate `document.status` and rot — keep state in the property. Triage operates on existing notes (`taking-notes` authors them) and tracks work shape, not who or when — no deadlines, calendars, or assignment.
