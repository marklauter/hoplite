...
Write `docs/journal/<YYYY-MM-DD>-<HHMM>-<slug>.md` to the frontmatter standard (`${CLAUDE_PLUGIN_ROOT}/references/frontmatter.md`): `title`, `type: journal`, `summary`. No `status` — entries don't evolve. Open with the context going in, then what you tried, expected, and learned.
