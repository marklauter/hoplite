...
- Blocked → link it. Todos that must close first → a `blocked_by` edge property whose value is a quoted wikilink, like `blocked_by: "[[<target>]]"` (a block-style list when there are several; edge/link syntax: `${CLAUDE_PLUGIN_ROOT}/references/expressing-edges.md`).
