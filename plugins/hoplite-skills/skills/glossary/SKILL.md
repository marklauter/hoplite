---
name: glossary
description: Reduce a resolved term to its kernel and write it to docs/hoplite/glossary/.
...
Write `docs/hoplite/glossary/<term>.md` (kebab-case) to the frontmatter standard (`${CLAUDE_PLUGIN_ROOT}/references/frontmatter.md`):
...
- The entry lives at `docs/hoplite/glossary/<term>.md`, the summary is the smallest phrase that unpacks the term, and the body opens with it verbatim.
- One sense survives: synonyms retired, retired page names carried in `aliases`.
- Edges are quoted wikilinks; every symmetric edge is reciprocated on its target.
- The term is indexed alphabetically in the glossary README `## Terms`.
- `status: locked` only if reduced *and* resolved; otherwise `evolving`.
