...
- Action-shaped but untyped → sweep. A note that reads as an action item but lacks `type: todo` → type it, move it to `docs/todos/`, then triage.
...
`closed`/`resolved` as tags duplicate `status` and rot — keep state in the property. Triage operates on existing todos (the `todo` skill authors them) and tracks work shape, not who or when — no deadlines, calendars, or assignment.
