...
class TestMarkdownIn:
    def test_finds_the_documents_directly_in_the_folder(self, tmp_path: Path) -> None:
        ...
        assert [path.name for path in markdown_in(real(tmp_path), tmp_path)] == ["a.md", "b.md"]

    def test_an_empty_folder_holds_no_documents(self, tmp_path: Path) -> None:
        assert markdown_in(real(tmp_path), tmp_path) == ()
    ...
    def test_a_directory_named_like_a_document_is_not_one(self, tmp_path: Path) -> None:
        ...
        assert [path.name for path in markdown_in(real(tmp_path), tmp_path)] == ["real.md"]
    ...
    def test_a_dangling_link_is_still_a_document(self, tmp_path: Path) -> None:
        ...
        assert [path.name for path in markdown_in(real(tmp_path), tmp_path)] == ["dangling.md"]
...
class TestOtherFiles:
    ...
    def test_a_hidden_markdown_file_is_still_a_document(self, tmp_path: Path) -> None:
        ...
        assert other_files(real(tmp_path), tmp_path / "docs") == ()
        assert [path.name for path in markdown_in(real(tmp_path), tmp_path / "docs")] == [
            ".draft.md"
        ]
...
class TestSubdirectories:
    def test_lists_child_directories_in_order(self, tmp_path: Path) -> None:
        for name in ("z", "a", "m"):
            (tmp_path / name).mkdir()
        _write(tmp_path / "a.md", "")
        assert [path.name for path in subdirectories(real(tmp_path), tmp_path)] == ["a", "m", "z"]
    ...
    def test_hidden_directories_are_never_walked_into(self, tmp_path: Path, name: str) -> None:
        ...
        (tmp_path / name).mkdir()
        (tmp_path / "docs").mkdir()
        assert [path.name for path in subdirectories(real(tmp_path), tmp_path)] == ["docs"]

    def test_a_dot_inside_the_name_is_not_hidden(self, tmp_path: Path) -> None:
        (tmp_path / "v1.2").mkdir()
        assert [path.name for path in subdirectories(real(tmp_path), tmp_path)] == ["v1.2"]
...
class TestWhichNameIsDescended:
    ...
    def test_the_link_sorting_first_does_not_win_the_descent(self) -> None:
        assert walk(fake(self.FILES), self.DOCS) == (
            Directory(path="docs", depth=0, documents=0),
    ...
    def test_the_recursive_listing_uses_the_paths_the_corpus_addresses(self) -> None:
        assert [
            document.path for document in collect(fake(self.FILES), self.DOCS, recurse=True)
    ...
    def test_naming_the_link_outright_still_walks_it(self) -> None:
        ...
        assert [node.path for node in walk(fake(self.FILES), self.DOCS / "aaa")] == [
            "docs/aaa",
            "docs/aaa/deep",
        ]
    ...
    def test_a_link_to_a_directory_no_other_path_reaches_is_walked(self) -> None:
        ...
        assert walk(fake(files), self.DOCS) == (
            Directory(path="docs", depth=0, documents=0),
            Directory(path="docs/specs", depth=1, documents=1),
        )
        assert [document.path for document in collect(fake(files), self.DOCS, recurse=True)] == [
...
class TestUnlistableDirectory:
    ...
    def test_it_is_not_descended_into(self) -> None:
        ...
        walked = [node.path for node in walk(fake(self.FILES), self.DOCS)]
    ...
    def test_it_costs_the_recursive_read_only_its_own_documents(self) -> None:
        ...
        collected = collect(fake(self.FILES), self.DOCS, recurse=True)
    ...
    def test_asking_for_it_directly_reports_rather_than_fails(self) -> None:
        assert collect(fake(self.FILES), self.CLOSED) == ()
        assert other_files(fake(self.FILES), self.CLOSED) == ()
        assert walk(fake(self.FILES), self.CLOSED) == (
...
class TestContainmentWithoutSymlinks:
    ...
    def test_a_directory_linking_inside_the_corpus_is_listed_once(self) -> None:
        ...
        walked = walk(fake(self.FILES), self.DOCS)
    ...
    def test_a_document_linking_out_of_the_corpus_is_refused_by_name(self) -> None:
        collected = collect(fake(self.FILES), self.DOCS)
...
class TestCollectContainment:
    ...
    def test_a_document_whose_target_cannot_be_reached_is_named_not_raised(self) -> None:
        ...
        assert collect(fake(files), docs) == (
            Unreadable(
                path="docs/loop.md",
