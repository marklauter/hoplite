...
The plan assumed the locked specs could stay at `docs/specs/` while the skills cited them by repo path. That broke on first contact: an installed plugin resolves nothing outside its own tree. Mark inverted the dependency. The specs moved into the plugin at `plugins/hoplite-skills/references/` as the source of truth, and `docs/specs/frontmatter.md` and `docs/specs/expressing-edges.md` became relative symlinks pointing in. Corpus wikilinks kept resolving; the plugin became self-contained. Expected risk: git on Windows checks out symlinks as text files unless `core.symlinks=true` and Developer Mode hold. Both held here. PowerShell's `New-Item -ItemType SymbolicLink` resolved the relative target against the working directory instead of the link location, so the links went in with `cmd /c mklink`.
