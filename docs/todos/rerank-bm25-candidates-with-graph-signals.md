...
- [[docs/todos/related-edges-rarely-fire-in-current-corpus.md]] — load-bearing dependency. The graph prior loses its inferred-adjacency signal until MinHash edges actually fire.
- [[docs/todos/mcp-reference-undersells-the-graph-against-grep.md]] — adjacent intervention. That note is about *prose framing* to redirect agents to existing tools; this note is about an *algorithmic* upgrade to the matching itself. Both make Hoplite categorically different from grep, at different layers.
