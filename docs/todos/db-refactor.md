...
See [[docs/todos/reify-in-memory-graph-as-file-based-sqlite.md]] for the why and the trigger that fired this work. This note is the how.
...
- Schema source. The canonical DDL lives at `plugins/hoplite/mcp/src/hoplite/schema.sql`. It loads once at import as `SCHEMA`. Current shape: `node`/`node_property`, `edge` with an interned `edge_kind` FK, `edge_property`, and the `fts` virtual table. `node.uri`, `node_property.key`, and `edge_kind.kind` carry `COLLATE NOCASE`.
- Case-insensitivity is the column's job. `node.uri COLLATE NOCASE` makes `WHERE uri = ?` case-insensitive. The old "casefold paths on store, casefold the input on resolve" contract is deleted everywhere; there is no casefold-on-store. The walker writes URIs verbatim.
...
- DB file path. `.hoplite/hoplite.schema.001.sqlite` at the corpus root. The schema version is embedded in the filename. Bumping it creates a new file alongside, so processes on different schema versions never trample each other. No `PRAGMA user_version`.
- Server startup does nothing. The MCP server boots with no DB I/O: no file creation, no schema apply, no walk. The first manual `refresh()` is the bootstrap. It opens the file read-write-create, applies the schema if tables are missing, and runs the walk. One bootstrap path; no startup race; no per-window cold start.
...
- Query tools fail loud if the index is missing. `where`/`relatives`/`export` open read-only (`file:<path>?mode=ro`). A missing file raises an explicit "no index — call `refresh` first" error, not a silent empty result.
...
- Tests use `:memory:`. Fixtures bootstrap a `:memory:` connection with the same schema, populate via the walker, and exercise the methods. The one exception is `open_ro` URI behavior, which needs a real file.
- `tools.py` depends on `Graph` directly. Handlers construct `Graph(db, corpus_root)` per call and delegate. Search and traverse logic that lived in `tools.py` (BFS, `_neighbors`, FTS query, escape) moves onto `Graph`. The handler bodies shrink to predicate parsing plus a single `graph.search(...)` / `graph.traverse(...)` call.
- No `hoplite init` step. `refresh` is the init step.

## Schema/model vocabulary drift — open reconcile

`schema.sql` has moved to `node`/`uri`/`edge_kind`, but `models.py` and `row_factories.py` still use `Document`/`path`. The queries in [[docs/notes/graph-py-design.md]] bridge it by projecting `n.uri AS path`, so the landed factories keep working unchanged. This is a deliberate, contained drift. A later pass either renames the models (`Document` to `Node`, `path` to `uri`) to match the schema and the "addressable byte resource / medium-agnostic identity" framing in `docs/specs/hoplite-architecture.md`, or accepts the `AS path` alias as permanent. This is not resolved in this refactor; it is flagged so it isn't forgotten.

## Numbered steps

Work in this order. Each step is shippable on its own — tests pass.

1. `db.py` — `Database` interface plus `FileDatabase` impl. Design at [[docs/notes/db-py-design.md]]. Done 2026-05-27. Landed with passing tests covering pragmas, URI handling, parent-dir auto-create, both domain errors, the `write_transaction` commit/rollback/busy paths, and (added later) crash-orphaned-WAL recovery under a read-only connection.
...
- Reconcile semantics. Replace truncate-and-rebuild with a three-set diff against the filesystem: added (insert), removed (cascade-delete), changed (`content_hash` differs, so re-extract). Cheap mtime check first. Unlocks incremental reindex.
...
- Connection pooling vs locking. Day-one is connection-per-call behind the `Database` interface. If open/pragma overhead proves meaningful under concurrent load, the options are:
  - Lock around a single shared connection. Simplest, but defeats WAL: concurrent reads serialize and the N-readers win disappears. Only right if concurrency is genuinely low.
  ...
  - The pool is the right direction when the bottleneck is real. Either swap-in lands behind the existing `Database` interface, with no handler or refresh-path changes.
