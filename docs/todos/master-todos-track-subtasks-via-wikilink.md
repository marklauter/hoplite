...
- [[docs/todos/add-contradicts-as-an-authored-edge-kind.md]] and [[docs/todos/add-not-related-as-a-structural-negative-edge-kind.md]] — the stereotype precedent. The open stereotype vocabulary `subtask` would join.
- [[docs/todos/hoplite-predicates-are-hql-rewrites-over-typed-relations.md]] — once HQL lands, the rollup rule reads naturally as a computed relation over sub-task status.
