...
docA → { tag:note, tag:minhash, prop:status=design,
         ...
         folder:docs/specs, created-month:2026-06 }
```
...
[[docs/todos/rank-related-edges-by-bm25-cosine-not-minhash-jaccard.md]] applies the same rarity weighting to the content channel alone — BM25 cosine over FTS tokens, where IDF supplies the rare-term weight that plain MinHash Jaccard washes out by voting every shingle equally.
...
- Whether this reframes the three-channel structure in [[docs/specs/hoplite.md]] Discover into a single unified-feature similarity, with the channels demoted to feature groups.
...
- [[docs/todos/rank-related-edges-by-bm25-cosine-not-minhash-jaccard.md]] — the content-channel special case; IDF weighting for text, which this generalizes to all features.
- [[docs/todos/rerank-bm25-candidates-with-graph-signals.md]] — adjacent: combines text and graph signals as a query-time rerank, where this note combines them at edge-construction time.
- [[docs/todos/weighted-edge-traversal-ranks-by-accumulated-similarity.md]] — once the edge score is a unified rarity-weighted similarity, path accumulation rides cleaner signal.
