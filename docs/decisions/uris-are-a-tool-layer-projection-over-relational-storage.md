...
Two registers, with one translation layer between them.
...
- Inside is the normalized schema: `node`, `node_property` + `property_key`, `node_tag` + `tag`, `edge` + `edge_stereotype` + `stereotype`. The model is explicit and self-documenting.
...
- Single-table design is a workaround for constraints SQLite does not have. It exists because DynamoDB has no joins and bills per request across a network, so every access pattern is pre-baked into one partition. SQLite joins are free, in-process, and planner-optimized. Porting the discipline imports its tax (rigid pre-enumerated patterns, amplified writes, model hidden in keys) to solve a problem that is absent here.
- The relational schema is the model canon. Collapsing to `item(pk, sk, data)` moves the model out of the DDL and into key-string conventions plus loader code; `schema.sql` stops describing the model. For a documentation-first design that is a regression.
- The reads that matter stay relational regardless: full-text search is an `fts5` virtual table, and graph traversal is a recursive CTE. So the store is never actually single-table, and the things key-value would optimize are non-problems.
- The access patterns are still in flux (survey, match, walk are half-specified), so baking them into amplified items now is premature.

The decision: keep relational storage as the model-of-record; keep uris as the tool-layer projection. If access patterns later stabilize and reads get hot, a key-value read-projection can be materialized at rebuild time as a layer on top. It stays a derived optimization, never the source of truth.
...
- [[docs/specs/hoplite-graph.md]]: the relational model of record.
