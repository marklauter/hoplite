...
[Inference] If the `related` edge fired at the documented `DEFAULT_THRESHOLD = 0.20` Jaccard over MinHash signatures, the two journal entries from `2026-05-25` on the tag-model refactor share enough vocabulary and structure to clear that bar. They didn't.
...
3. **Tokenization collapses too aggressively.** [Guess] If `minhash.signature` shingles by character n-grams or whitespace tokens without stopword handling, the markdown skeleton (headings, bullets, link syntax) may dominate the signature and wash out content overlap.
4. **MinHash pass not actually emitting.** [Guess] The aggregate pass at `graph.py:363,450` may be silently dropping edges — bytes-decoding round-trip, signature comparison off-by-one, or threshold inversion.
