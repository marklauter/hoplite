...
Refactor writing-* skills so writing-documentation is the shared foundation and downstream skills (taking-notes, journaling, writing-wiki, reviewing-wiki) compose on top of it.

## Observation

Current prose-artifact skills are split philosophy / guidance / validation within each skill, with each skill restating general editorial rules in its own words. `writing-documentation`, `writing-wiki`, `taking-notes`, and `journaling` overlap on universals (positive framing, dedup, link discipline) while diverging on specifics (audience, tone, register, voice, intent). Paired writing/reviewing skills (`writing-wiki` / `reviewing-wiki`) can drift because nothing structurally couples them to a single philosophy.

## Interpretation

Abandon the per-skill philosophy/guidance/validation split in favor of foundation + downstream composition.

- `writing-documentation` becomes the foundation. It states the general laws of good writing (positive framing, etc.) and declares that audience, tone, register, voice, and intent MUST be set by the downstream. It is unaware of any specific downstream.
...
- Rewrite `writing-documentation` as a foundation: enumerate the editorial laws with MUST / SHOULD / MAY markers; declare the audience/tone/register/voice/intent slots downstreams must fill.
- Add an explicit "load `writing-documentation` first" instruction at the top of each downstream SKILL.md, and hint at the dependency in the frontmatter description for discovery.
...
- Decide whether `reviewing-documentation` becomes a sibling foundation for review skills or just inherits directly from `writing-documentation`.
