...
Claim keys are predicates. `status`, `tags`, `created`, `summary` sit in the same open vocabulary as `cites` and `supersedes` — the middle position is one kind of thing. This supersedes the edges-only scope choice from the terminology alignment (see the journal entry below). The `predicate`↔`claim` glossary contrast needs redrawing — held for the read-side lock pass.
