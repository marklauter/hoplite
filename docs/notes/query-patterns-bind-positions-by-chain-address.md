...
- **Position names** — `subject`, `predicate`, `object` — name the three statement positions. They are query grammar, not namespaces: `predicate` names the middle *position*, which a [[docs/hoplite/glossary/relationship.md|relationship]] label or a claim key may fill.
...
Open threads: composition (several patterns, boolean structure — this is the concrete-syntax seed for the [[docs/hoplite/glossary/search-expression.md|search expression]]); whether unbound positions can be named for multi-position results; how a pattern narrows a [[docs/hoplite/glossary/walk.md|walk]]; and the position names themselves — `subject`, `predicate`, `object` occupy the short-form token space, so the grammar either reserves them in query context or marks positions distinctly (a key coined `predicate` would make `:predicate:status` ambiguous).
