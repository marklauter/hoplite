# Write writing-skills and writing-bash skills with verification sections

Tags: todo,skills,writing-skills,writing-bash
Two new skills to create; each must include a Validation section that names running the existing test suite, so the audit-mode test miss does not repeat.
...
- Two new skills to create:
  - `writing-skills` — the meta-skill about authoring SKILL.md files (four-section structure, Philosophy/Guidance scope, named-anchor philosophy intro, intro discipline).
...
- Draft writing-skills with the four-section structure as the content. Validation section names: read sibling SKILL.md files, run any existing test suite for the skill's scripts, verify `--help` on every script.
