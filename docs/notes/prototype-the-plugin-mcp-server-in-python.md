...
Locked in: Shape C, native Python with bash deprecated. The MCP server is the only implementation; the bash scripts (`record-note.sh`, `record-entry.sh`) are gone. Hoplite ships under `plugins/hoplite/mcp/` as the source-of-truth runtime. See [[docs/hoplite/hoplite-architecture.md]] for the implementation shape that landed.
