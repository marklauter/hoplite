...
[Locked in] Shape C — native Python, bash deprecated. The MCP server is the only implementation; bash scripts (`record-note.sh`, `record-entry.sh`) are gone. Hoplite ships under `plugins/hoplite/mcp/` as the source-of-truth runtime; see [[architecture]] for the implementation shape that landed.
