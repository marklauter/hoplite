...
- **N MCP processes start, all do nothing.** No race window at import. The race only exists when two processes both call `refresh()` simultaneously, and `BEGIN IMMEDIATE` serializes that cleanly per [[docs/notes/db-py-design.md]].
- **Per-window cold start is zero.** The whole "load the corpus from scratch every session" cost — which scaled linearly with corpus size and became the headline reason this refactor exists — disappears for every window after the first.
...
The headline win: a developer with 12 Claude Code windows open against the same corpus pays the walk cost *once*. Every other window opens the shared DB file, sees data already there, and serves queries immediately.
...
This is `tools.py`'s domain, but worth restating here so the server.py reader doesn't have to chase the cross-reference:
...
Implication for cross-version coexistence: when the schema bumps to v2, the new code computes the path as `hoplite.schema.002.sqlite` and creates a fresh file. The v1 file stays on disk; v1 processes opening that file continue to serve. Two MCP server versions can coexist on the same corpus without stomping on each other's data.
...
Tying together what's stated in [[docs/notes/db-py-design.md]] and [[docs/notes/tools-py-design.md]]:

- N server processes start concurrently. Each calls `tools.set_root` at import; each ends up holding its own `FileDatabase` instance pointing at the same file path. No file I/O yet.
...
- **Don't open the DB at import.** No `db.open_rw()`, no `migrations.apply()`, no walk. The whole point is that startup is free. If you find yourself wanting to "pre-warm" the connection or "verify the schema is current" at server boot, you're undoing the concurrency model.
- **Don't move `set_root` into a function called by the first handler.** Today's pattern (call at import time) is correct — the singletons are needed by every handler, and import is the right time to compute them. Lazy initialization here doesn't buy anything because import is fast.
- **Don't add an `init` MCP tool.** `refresh` is the init. Adding a separate `init` gives users a second thing to remember to call, and adds a code path that does what `refresh` already does.

### Known gaps — accepted, documented, not yet fixed

- **First refresh against an existing v1 file when the v2 server starts** — the new server opens `hoplite.schema.002.sqlite`, which doesn't exist, and creates a fresh v2 file. The v1 file stays on disk; the user pays a fresh walk on v2's first refresh. This is by design but worth flagging during release notes.
- **Server crash during a refresh** — `BEGIN IMMEDIATE` rollback unwinds the partial walk, so the DB stays in its prior state. A surviving process picks up where the dead one left off (its own next `refresh` call). No corruption, no "stuck lock" state — SQLite's WAL handles this via the journal.
