...
| Node kind | Address | Predicates | Behavior |
|---|---|---|---|
| Shared value node | the value itself: `status/locked`, `tag/note`, `created/2026-06-30` | categorical, multi-document, slug-safe | shared and walkable — "who else carries this value"; range queries ride lexicographic uri scans (ISO-8601 dates sort) |
| Slot node | the subject's uri: `summary/<doc-uri>`, `title/<doc-uri>`, `minhash/<doc-uri>` | functional (one per document), freeform text or blob | stable citation with live content; rename hazard covered by the existing alias machinery |
