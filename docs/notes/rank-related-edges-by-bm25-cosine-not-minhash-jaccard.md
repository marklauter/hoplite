...
Option (1) is the faster path to a working prototype because the score function already exists. Option (2) composes more naturally with [[docs/notes/rerank-bm25-candidates-with-graph-signals.md]]: that note covers BM25 candidate retrieval for `where`, and BM25-weighted vectors give one representation reusable for both jobs.
...
At small scale (≤ 1000 docs), the candidate-gen step adds no value; compute BM25 cosine across every pair. The hybrid pays off at the multi-vault scale covered in [[docs/notes/configurable-corpus-vaults-replace-single-root.md]].
