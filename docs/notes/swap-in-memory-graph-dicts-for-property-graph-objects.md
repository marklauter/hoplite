...
- Multi-property predicates that today require self-joins on the dump (see [columnar projection in roadmap](../mcp/roadmap.md#columnar-projection-for-multi-property-predicates)) — an in-memory inverted index would serve them without leaving Python.
