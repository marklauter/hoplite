...
After `/plugin install hoplite@msl.hoplite` (or any rename or reinstall flow that produces a fresh `${CLAUDE_PLUGIN_DATA}` directory), the MCP supervisor reports `Failed to reconnect to plugin:hoplite:hoplite: MCP server connection timed out after 30000ms`. The data directory at `~/.claude/plugins/data/hoplite-msl-hoplite/` exists but is empty — no `venv/`, no `pyproject.toml` snapshot.
