...
- **`docs/hoplite/hoplite-authoring.md`** — title and H1 read *"Authoring — declare relationships, describe documents"*; body: *"two mechanisms: declare and describe"* and the `## Declare and describe — applying explicit structure` section. Rewrite around the single `assert` act (a stub, so cheap).
