...
- **title** is a property whose value defaults to the slug (the filename, dashes as spaces), so it need not be written in frontmatter — but it is a property nonetheless.
