...
- Untested scripts by location:
  ...
  - `plugins/skills/skills/writing-csharp/scripts/`: `build.sh`
...
- **Probably skip at this layer**: `writing-csharp/scripts/build.sh` orchestrates `dotnet format`/`build`/`test` against a real .NET project. Integration-style; unit-testing the orchestration doesn't catch much.
...
- Skip `build.sh` for now; revisit if it grows non-trivial logic that isn't `dotnet` orchestration.
