...
This document covers the system as one piece. Tool signatures and the 4-tool API live in [[docs/specs/hoplite-tool-api.md]]; deferred features in [[docs/specs/hoplite-roadmap.md]].
...
Pairwise scan is O(N²) but cheap at scale — 128-int Jaccard comparisons run in ~100ms for 1000 documents. LSH bucketing is the optimization to reach for past 10⁵ documents; see [[docs/specs/hoplite-roadmap.md]].
...
Per-query stat-checking is the most-likely future upgrade (see [[docs/specs/hoplite-roadmap.md]]).
...
Single writer, single reader, single server process. The in-memory graph isn't safe for concurrent mutation; the MCP server's single-threaded request handler is the lock. Multi-writer support is deferred ([[docs/specs/hoplite-roadmap.md]]).
