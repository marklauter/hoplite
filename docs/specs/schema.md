...
Every position of a statement holds a resource. Addresses are namespace chains grounded at `meta:meta`; relationships and claims are the kinds licensed for the predicate position. The model is [[docs/notes/every-triple-position-is-a-resource.md]]; how it settled is [[docs/journal/2026-07-02-0139-the-reversal-every-triple-position-is-a-node.md]] (recorded under the old title); the term crosswalk is in [[docs/glossary/README.md]]. The pre-reversal property-graph schema is preserved in git history.
