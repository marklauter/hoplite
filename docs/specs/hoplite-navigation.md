...
Find reads the namespace — `property_key` on the document side, `stereotype` on the edge side, each a small interned list (see [[docs/specs/hoplite-graph.md]]).
...
1. Anchor on features, not structure. The lede should open on the move querying features, not "the structure affords five moves" — affordances arise from features; the graph is the mechanism beneath them ([[docs/specs/hoplite-feature-taxonomy.md]]). Read affordances range over the whole feature set.
