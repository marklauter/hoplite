...
Entities referenced below — `Document`, `Edge`, `Hit`, `TraversalHit`, `WriteResult` — and the rules around frontmatter, wikilink resolution, ghost documents, and edge derivation are documented in [[docs/specs/hoplite-architecture.md]].
...
The two query tools accept a tag predicate that filters which documents appear in results. The predicate is a string with the grammar defined in [[docs/specs/hoplite-architecture.md#tag-predicates]].
...
No pagination day one. The `k` cap is the result bound; the agent picks `k` to match how much it wants to look at. Pagination is on the [[docs/specs/hoplite-roadmap.md|roadmap]].
...
Day one this is the only way to pick up file changes between queries. Agents that write `.md` files call `refresh` afterward to make the new content visible. Human edits in Obsidian show up after the next reindex call. See [[docs/specs/hoplite-architecture.md#the-walker]] for the walk's two-pass shape.
...
One-shot operation. The schema is a byte-for-byte mirror of in-memory state — `document`, `document_property`, `edge`, `edge_property`, plus an FTS5 index over `path`, `title`, `summary`, and `body` replayed from the live FTS connection. Full DDL in [[docs/specs/hoplite-architecture.md#dump-schema]].
...
Per [[docs/specs/hoplite-architecture.md#error-model]], invariant violations throw exceptions (programming errors the caller could have prevented — `None` for a required string, malformed predicate); constraint violations ride along inside successful results as warnings (frontmatter parse failures, unwritable dump destinations).
