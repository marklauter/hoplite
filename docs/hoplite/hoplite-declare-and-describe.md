...
To be written. Absorbs the express-only contract currently in [[docs/hoplite/hoplite-frontmatter.md]] (held pending the data-model spec): namespaced keys, bare `title`/`summary`, mandatory and optional fields, the spelling axes, malformed-block handling, Obsidian compatibility.
