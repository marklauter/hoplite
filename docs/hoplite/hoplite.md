...
The agent surveys the vocabulary, encoded in the map, and queries the corpus in its own terms. It asks informed questions instead of guessing which string to `grep`. Search matches on meaning rather than a literal string. The agent analyzes a rich result set, not a sampled excerpt. Hits are ranked by IDF-weighted Jaccard over a unified feature set: content, metadata, and neighborhood. Relatedness surfaces in the ranking. Hard-won groundwork ranks among the results, no longer dark. The agent receives the signal it would have missed and trusts nothing is hidden.
