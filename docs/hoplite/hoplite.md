...
Scanned noise biases the agent; so does signal it never reaches. `grep` requires the agent to name the answer before it can search for it. Explore agents, guarding their context budget, scan excerpts and risk missing critical information — valuable terms for the follow-up search. Hard-won memory sits unused. The knowledge-starved agent exercises poor judgment: it reopens settled decisions, contradicts conventions, overwrites working code, and reproduces prior art. Failures compound.
