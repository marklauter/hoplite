...
The agent surveys the vocabulary, encoded in the map, and queries the corpus in its own terms. It asks informed questions instead of guessing which string to `grep`. Search matches on meaning rather than a literal string. The agent analyzes a rich result set, not a sampled excerpt. Hits are ranked by rarity-weighted Jaccard over a unified feature set. Content, metadata, and neighborhood all count at once. Relatedness surfaces in the ranking. Hard-won groundwork ranks among the results, no longer dark. The agent receives the signal it would have missed and trusts nothing is hidden.
...
`grep` matches only what the agent can name. The map's vocabulary is discoverable. The agent surveys the metadata asserted within the corpus — tags, properties, and stereotypes — and queries in the corpus' own terms. Beyond what it can name, it discovers: from any hit the agent walks to the neighborhood — the edges the author declared, the documents that point back, and the kin the engine discovered. Ghosts keep the unwritten enumerable. Hard-won memory sits one hop away. The agent reaches what it never thought to name.