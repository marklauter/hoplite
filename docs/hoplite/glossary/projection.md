...
- **Order** — sort the hits by the relevance score a scoring predicate attached, or by an intrinsic value like recency or centrality. [[match]] returns an unordered set and a scoring [[semantic-search]] predicate only attaches a score; projection is the only operation that imposes an order.
