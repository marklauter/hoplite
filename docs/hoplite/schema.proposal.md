...
created: 2026-07-08
status: evolving
refines: "[[docs/hoplite/schema.md]]"
---

# Schema proposal — flat scoping replaces recursive namespaces

A proposal against [[docs/hoplite/schema.md]]: keep the triple store, drop the recursive namespace dictionary. The namespace chains were built to prevent name collisions, but Hoplite does not govern the vocabulary — the user owns the schema, as [[docs/notes/predicates-are-an-open-vocabulary.md]] already rules. Prevention by addressing scheme was governance smuggled in through the resolver. Let names collide; scope the one place scoping pays.
...
The fourth kind dissolves the relationship/claim split in [[docs/hoplite/schema.md]]. `cites` is just a property whose asserted objects happen to be documents. Predicate licensing reduces to: the predicate position holds a property.
